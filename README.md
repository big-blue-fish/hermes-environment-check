...
这是一套系统化的 Hermes Agent 环境健康检查与修复技能。当你怀疑环境缺失组件、工具不可用、配置被破坏，或需要安装/升级 Hermes 时使用。覆盖 Windows 专属问题：编码、PATH、winget、文件系统、MCP 服务器诊断与故障排查。已在多个版本的Hermes-CN-Desktop上测试过。
